(() => {
  ...
  function AvatarGroup({ items = ContextEditorUIConfig.components.avatarGroup.demo.items, label = '头像组', id = 'avatar-group' } = {}) {
    ...
    items.forEach((item, index) => {
      ...
      const image = document.createElement('img'); image.src = new URL(item.src, assets).href; image.alt = item.alt || `头像 ${index + 1}`; image.width = item.width || 120; image.height = item.height || 120
