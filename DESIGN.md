...
- 子代理使用无边框的行内布局, 固定图标搭配不同颜色与名称, 与工具调用保持相同的图文起点
...
- 窄屏重新分配控件宽度, 避免压缩导致重叠
- 发送后在消息展开与输入框高度稳定后滚动, 让新消息末尾和时间信息位于输入框上方, 连续发送以最后一条为准
...
- 同一子代理在开始和完成时保持相同名称与颜色, 状态紧随名称, 换行保持文字对齐
...
- 操作反馈使用 120ms, 面板进入使用 180ms, 展开与排序使用 260ms, 退出使用 120ms, 时长和缓动读取共享配置
- 弹出面板从对应按钮上方进入, 折叠区域连续改变高度, 条目排序保留位置关系
- 退出中的内容立即停止交互, 结束后移除, 快速反向操作从当前动画位置继续
- 静态资料标签与头像不添加循环或悬停动效, 完成状态不伪装成持续处理
- 窄屏不出现重叠, 裁切或水平滚动
